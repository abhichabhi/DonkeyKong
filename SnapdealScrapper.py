...
class SnapdealScrapper():
	reviewBaseURL = "http://www.snapdeal.com/product/[productid]/reviews?page="
	...
	def __getLastUpdatedDate():
		...
		allReviewDatesForProduct = mongoReviewClient.getItems(constants.SNAPDEAL, productName=self.productName)

	def __scrapeReview(self,baseURL):
		# mongoReviewClient = self.mongoReviewClient.dbClient
		# phoneEntry = mongoReviewClient[constants.SNAPDEAL].find_one({"productName":self.productName, "productID": self.productID})
		# latestDate = "0"
		# if not phoneEntry:
		# 	mongoReviewClient[constants.SNAPDEAL].insert({"productName":self.productName, "productURL": self.pDpUrl, "productID": self.productID})
		# 	phoneEntry = mongoReviewClient[constants.SNAPDEAL].find_one({"productName":self.productName, "productID": self.productID})
		# 	phoneEntry['reviews'] = {}
		# else:
		# 	print phoneEntry
		...
		while(1):
			...
			if self.__isEndOfReviews(hxs):
				...
				pageIter = pageIter + 1
			postdate = "9999-99-99"
			reviewBlock = hxs.select('//div[contains(@class,"commentlist")]').extract()
			if reviewBlock == []:
			...
			for reviews in reviewBlock:
				...
				# mongoReviewClient[constants.SNAPDEAL].save(phoneEntry)
			if self.todate > postdate:	
				break
	...
	def __getDateTextFromReviewBlock(self, reviews):
		...
		postdate = reviews.select('//div[contains(@class,"date")]/text()').extract()
		postdate = datetime.datetime.strptime(postdate[0],"%b %d, %Y").date()
		reviewtext = reviews.select('//div[@class="user-review"]/p/text()').extract()
	...
	def __getReviewURL(self):
		...
		if "?" not in pdpURL:
			pdpURL = pdpURL + "?"
		productID = self.__extract_between(pdpURL + "", '/', '?', 5)
